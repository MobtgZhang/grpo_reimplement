...
import numpy as np
def draw_loss(losses_list, save_path, title="Training Loss Curve", xlabel="Steps", ylabel="Loss"):
    """
    ...
    save_file_name = os.path.join(save_path, "loss")
    plt.savefig(f"{save_file_name}.jpg", dpi=300, bbox_inches='tight')
    plt.savefig(f"{save_file_name}.pdf", bbox_inches='tight')
    plt.close()
if __name__ == '__main__':
    save_path = "./checkpoints/Qwen2.5-7B-Instruct"
    import json
    for file_path in os.listdir(save_path):
        load_loss_file = os.path.join(save_path, file_path, "loss_list.json")
        save_model_path = os.path.join(save_path, file_path)
        with open(load_loss_file, "r") as f:
            losses = json.load(f)
        draw_loss(losses, save_model_path)
